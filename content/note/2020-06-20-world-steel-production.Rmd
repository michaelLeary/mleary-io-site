...
Time series data: Steel production world (millions metric tons)

####
Source for data: [World Steel Association](https://www.worldsteel.org/media-centre/press-releases/2020/worldsteel-short-range-outlook-june-2020.html)
...
dataxl <- ('/Users/michaelleary/Documents/GitHub/indiairon/data/worldcrudesteelproduction.xlsx')
...
datacsv <- read_csv('/Users/michaelleary/Documents/GitHub/indiairon/data/worldcrudesteelproduction.csv', ) # in tibble form
```
...
```{r data_ts_form, include=FALSE}
#steep production as time series
#make template variable
ds <- datacsv
ds
ds <- ds[,-c(2)]
dsts <- ts(ds, frequency=1, start=c(1900), end=c(2021))
dsts
```
...
tmp = window(dsts, start=c(2000), end=c(2005))
class(tmp)
tmp
plot.ts(tmp)
```
...
plot.ts(dsts[,2],
        ...
        main="World Steel production 1900-2021e")
```
